import * as THREE from 'three';
    ...
    let localModel, localMixer, localRig;
    ...
    /* ---- Character library ----
     * Xbot and Ybot are both standard Mixamo rigs (identical `mixamorig:*` bone names), so a
     * single shared idle/walk/run animation library — loaded once from Xbot.glb — can be applied
     * to either mesh interchangeably; that's how Ybot gets a full walk/run cycle even though its
     * own .glb only ships one baked clip.
     *
     * The mannequin uses a different, Unreal-Engine-style skeleton (pelvis/spine_01/clavicle_l/...)
     * and ships only a single 'Male_commando_Idle_2' clip. Its bone names don't match the Mixamo
     * rig, so the shared idle/walk/run library can't be retargeted onto it without a proper bone
     * mapping. Until a matching walk/run export on this same skeleton is available, the mannequin
     * holds its idle pose while moving — translation/rotation/multiplayer sync all still work, it
     * just won't show a walk cycle. See CHARACTER_LIBRARY[...].rig below.
     */
    const CHARACTER_LIBRARY = {
        mannequin: { label: 'Mannequin', url: 'assets/models/creative-mannequin.glb', rig: 'other' },
        xbot: { label: 'Xbot (female)', url: 'assets/models/Xbot.glb', rig: 'mixamo' },
        ybot: { label: 'Ybot (male)', url: 'assets/models/Ybot.glb', rig: 'mixamo' }
    };
    const DEFAULT_CHARACTER_KEY = 'mannequin';
    ...
    // Shared idle/walk/run AnimationClip set for any Mixamo-rig character, loaded once and reused.
    let mixamoClipsPromise = null;
    function loadMixamoClips() {
        if (!mixamoClipsPromise) {
            mixamoClipsPromise = new Promise((resolve, reject) => {
                new GLTFLoader().load(CHARACTER_LIBRARY.xbot.url, (gltf) => {
                    const clips = { idle: null, walk: null, run: null };
                    gltf.animations.forEach(clip => {
                        const n = clip.name.toLowerCase();
                        if (n.includes('idle') && !clips.idle) clips.idle = clip;
                        else if (n.includes('run') && !clips.run) clips.run = clip;
                        else if (n.includes('walk') && !clips.walk) clips.walk = clip;
                    });
                    resolve(clips);
                }, undefined, reject);
            });
    ...
    // Loads any character from CHARACTER_LIBRARY and wires up idle/walk/run actions where the
    // skeleton allows it. Returns { model, mixer, actions, rig }.
    async function loadCharacterModel(key) {
        ...
        if (def.rig === 'mixamo') {
            const clips = await loadMixamoClips();
            Object.keys(actions).forEach(k => {
                if (!clips[k]) return;
                const action = mixer.clipAction(clips[k]);
                actions[k] = action;
                action.play();
                action.setEffectiveWeight(k === 'idle' ? 1 : 0);
            });
        } else {
            // Non-Mixamo rig: only bind clips that are actually authored for this skeleton.
            gltf.animations.forEach(clip => {
                if (clip.name.toLowerCase().includes('idle') && !actions.idle) {
        ...
        return { model, mixer, actions, rig: def.rig };
    }
    ...
    function addRemotePlayer(id, nick, characterKey) {
        ...
        loadCharacterModel(key).then(({ model: newModel, mixer, actions, rig }) => {
            ...
            newModel.add(label);
            players.set(id, {
                model: newModel, mixer, actions, rig, nick, characterKey: key,
    ...
    async function switchLocalCharacter(key) {
        ...
        const { model, mixer, actions, rig } = await loadCharacterModel(safeKey);
